...
function App() {
  ...
  useEffect(() => {
    if (gameID !== '') {
      ...
      doc.onSnapshot(
        (docSnapshot) => {
          if (
            'MafiaPlayers' in docSnapshot.data() &&
            docSnapshot.data().MafiaPlayers.length === 0
          ) {
            setGameState('civilianwin');
          } else if (
            'NursePlayers' in docSnapshot.data() &&
            docSnapshot.data().NursePlayers.length === 0 &&
            'DetectivePlayers' in docSnapshot.data() &&
            docSnapshot.data().DetectivePlayers.length === 0 &&
            'CivilianPlayers' in docSnapshot.data() &&
            docSnapshot.data().CivilianPlayers.length === 0
          ) {
  ...
  if (gameState === 'landing') {
    return <Landing db={db} setGameID={setGameID} />;
    ...
    return (
      <Type>
        {gameState === 'dead' ? (
          <Type align="center" variant="h1" color="primary">
            You have died
          </Type>
        ) : (
          <div />
        )}
        <Spectator docSnapshot={docSnapshot} />
      </Type>
    );
